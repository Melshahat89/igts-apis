...
@section('content')
    @component(layoutForm() , ['title' => trans('becomeinstructor.becomeinstructor') , 'model' => 'becomeinstructor' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
         @include(layoutMessage())
        <form action="{{ concatenateLangToUrl('admin/becomeinstructor/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
		 ...
		 <div class="form-group {{ $errors->has("socialAccount") ? "has-error" : "" }}" > 
            ...
            <div class="form-group">
                <button type="submit" name="submit" class="btn btn-default" >
                    ...
                    {{ trans('home.save') }}  {{ trans('becomeinstructor.becomeinstructor') }}
                </button>
            </div>
        </form>
    @endcomponent

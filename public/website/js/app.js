...
var $courseDetailStreamer = $('#course_detail_streamer');
...
function showCourseStreamer(evt){
  var windowScroll = $(this).scrollTop();
  var $courseDetail = $('#course_detail');
  var calc = $courseDetail.offset().top + 200;
  if( windowScroll >= calc ) {
    $courseDetailStreamer.addClass('show');
  }else {
    $courseDetailStreamer.removeClass('show');
  }
}

// stars
$('.jq-stars').each(function(index, item){
  var opts = $(item).data('options'),
    settings = $.extend({strokeWidth: 0, activeColor: '#FFCB04', ratedColor: '#FFCB04',  useGradient: false,hoverColor: '#FFCB04',emptyColor:'#c5d0c0',starShape: 'rounded'}, opts);
  $(item).starRating(settings);
});


$doc.on('click', '.talk_video_play_button', function(evt){
  evt && evt.preventDefault();
  $(this).closest('.talk_video').removeClass('video_hidden');
  //var player = $(this).closest('.talk_video').find('#video-js');
  videojs('talk_video_js', {
...
$doc.on('click', '.toggle_collapse', function(evt) {
  evt && evt.preventDefault();
  var accordionId = $(this).attr("accordion-id"), numPanelOpen = $(accordionId + ' .collapse.show').length;
...
function openAllPanels(accordionId) {
  $(accordionId + ' .panel_collapse:not(".show")').collapse('show');
}
function closeAllPanels(accordionId) {
  $(accordionId + ' .panel_collapse.show').collapse('hide');
}
...
// active placehoder inputs
 ...
 var window_width = $win.width();
  ...
  if (window_width < 960) {
    $('[data-sticky="nav"]').trigger("sticky_kit:detach");
  } else {
    make_sticky();
  }
  ...
  function make_sticky() {
    $('[data-sticky="nav"]').each(function(index, item){
      var opts = $(this).data('plugin-option');
      $(item).stick_in_parent(opts)
   });
  }
  // end it
 ...
 // show streamer on course detail page om scroll
 if( $courseDetailStreamer.length){
  $win.on('scroll', showCourseStreamer);
 }

  //smoth scroll
  $doc.on('click', '.smooth_scroll', function (event) {
